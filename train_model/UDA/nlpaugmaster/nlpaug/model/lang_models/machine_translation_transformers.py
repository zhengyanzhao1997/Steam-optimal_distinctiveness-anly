...
#         return all_translated_texts
    ...
    def translate_one_step_batched(
        ...
        tokenized_texts = tokenizer(data, padding=True, return_tensors='pt', max_length=self.max_length,truncation=True)
        with torch.no_grad():
            input_ids = tokenized_texts['input_ids'].to(self.device)
            attention_mask = tokenized_texts['attention_mask'].to(self.device)
            translated_ids_batch = model.generate(
                input_ids=input_ids, attention_mask=attention_mask,
                max_length=self.max_length,do_sample=True,temperature=0.7,num_beams=1,
